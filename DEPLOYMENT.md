...
| `CAP_EGRESS_ALLOW_INTERNAL_HOSTS=true` | Operator opt-out allowing tenant `egress_allowlist` FQDNs that resolve to internal endpoints (`localhost`, `.svc`, `.cluster.local`, metadata, rebinding helpers); each accepted host is audit-logged and the engine's `egressDeny` CIDR list shrinks to metadata/link-local only. |
| `CAP_ALLOW_PRODUCTION_INTERNAL_EGRESS=true` | Required in release builds when `CAP_EGRESS_ALLOW_INTERNAL_HOSTS` is enabled: an explicit second acknowledgment that tenant egress to internal endpoints is intended in production. |
