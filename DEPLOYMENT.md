...
| `CAP_EGRESS_ALLOW_INTERNAL_HOSTS=true` | Operator opt-out: tenant `egress_allowlist` hosts matching internal hostname patterns (`localhost`, `*.svc`, `*.cluster.local`, `*.internal`, `metadata.*`, rebinding helpers like `*.nip.io`) are accepted instead of rejected; each accepted host is audit-logged. This is a hostname-pattern check, not a DNS resolution check. When any allowlisted host on an app matches such a pattern, the app's generated Cilium policy shrinks its `egressDeny` CIDR list to metadata/link-local only for every allowlisted name on that app — RFC1918/loopback/ULA space becomes reachable via any allowlisted FQDN, while metadata (169.254.0.0/16, fd00:ec2::254) and link-local stay denied. |
