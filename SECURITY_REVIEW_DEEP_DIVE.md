...
| 6 | 2026-08-17 | Third review round: Cilium egressDeny field corrected to `toCIDR` (SSA dry-run verified against the live CRD both ways — the old `toCIDRs` failed strict decoding, blocking every deployment); release-envelope omission now fails closed once a per-API baseline exists; same-version divergent keyrings refused (canonical-bytes compare); per-org keyring and per-store baseline read/compare/write serialized under flock; malformed baseline aborts instead of resetting the high-water mark | `security/deep-dive-fixes` |
