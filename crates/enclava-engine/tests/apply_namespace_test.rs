...
/// Round-5 review finding (Devin ANALYSIS_0001): trusted-managed namespaces
/// converge via the trusted path — apply_resource still selects `.force()`
/// there, and until now no test pinned that a CAP-owned (trusted) namespace
/// actually applies successfully rather than failing closed. This tower-fake
/// test pins both outcomes: trusted → SSA PATCH succeeds (with force, per
/// the trusted-manager contract); foreign → fails closed without force.
#[tokio::test]
async fn namespace_apply_trusted_managers_converge_with_force() {
    use axum::http::{Method, Request, Response, StatusCode};
    use enclava_engine::apply::engine::ApplyEngine;
    use enclava_engine::apply::generation::MutationGeneration;
    use enclava_engine::apply::namespace::apply_namespace;
    ...
    let client = kube::Client::new(
        service_fn(move |request: Request<Body>| {
            ...
            async move {
                ...
                let mut locked = state.lock().unwrap();
                if method == Method::GET && path.ends_with("/namespaces/cap-test-org-test-app") {
                    // Trusted namespace: every non-status manager is the CAP
                    // field manager, so the trusted path applies with force.
                    let resource = serde_json::json!({
                        ...
                        "metadata": {
                            ...
                            "managedFields": [{
                                ...
                                // status subresource entries are ignored by
                ...
                if method == Method::PATCH {
                    ...
                    let applied = serde_json::json!({
                        ...
                        "metadata": {
                            ...
                            "resourceVersion": "2",
                            "annotations": {
                                "enclava.dev/cap-provider-mutation-generation": "2",
                            },
                        },
                    });
                    return Ok(Response::builder()
    ...
    let applied = apply_namespace(&engine, &ns, MutationGeneration::new(2).unwrap())
        .await
        .expect("trusted namespace must converge via SSA");
    use kube::Resource;
    assert_eq!(
        applied.meta().annotations.as_ref().unwrap()["enclava.dev/cap-provider-mutation-generation"],
        "2"
    );
    let queries = state.lock().unwrap().patch_queries.clone();
    assert_eq!(queries.len(), 1, "exactly one SSA PATCH, got {queries:?}");
    assert!(
        queries[0].contains("force=true"),
        "trusted managers authorize force; got {queries:?}"
    );
}
